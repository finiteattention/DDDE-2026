...
The Norwegian Labour and Welfare Administration (Nav) handles a lot of information about citizens. Nav must comply with many different regulations about processing of personal information, for example GDPR. It is also required to document that compliance on a large scale.

It's unreasonable to expect software development teams, facilities management, benefits offices etc, to each have the expertise required to understand and document compliance with many different laws, and to keep up with legislative changes. Previous work at Nav has developed a self-assessment tool to help individuals or teams work through Nav's "success criteria" for compliance with a range of different legislations. The tool models the legislative material in a highly structured way, with requirements descriptions and success criteria being owned and kept updated in the tool by subject specialist "requirements owners".

Where there is potential risk to citizens' privacy rights, teams may also need to complete a Data Protection Impact Assessment (DPIA). DPIA involves identifying and estimating risk, then implementing measures to reduce it. Until recently, DPIA in Nav was documented, reviewed and signed off using a Microsoft Word template. Word as a format was challenging for users (many of whom are not familiar with DPIA or privacy legislation), for Nav's data privacy ombudsman who reviews DPIAs and returns them with feedback, and for risk owners in the organisation who need an up to date overview of their risk portfolio.
...
* Cognitive load as a lens for thinking about design, specifically in documentation tools and in general.
...
* Harnessing user pains to prioritise feature development and guide design choices.
